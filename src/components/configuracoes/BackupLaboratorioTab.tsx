...
export function BackupLaboratorioTab({ onMensagem }: Props) {
  ...
  const [ehMasterAdmin, setEhMasterAdmin] = useState(false);
  ...
  useEffect(() => {
    void (async () => {
      const [resProp, resMe] = await Promise.all([
        fetch("/api/backup/seguranca-restaurar", {
          credentials: "same-origin",
        }),
        fetch("/api/auth/me", { credentials: "same-origin" }),
      ]);
      const proprietario = resProp.ok;
      setEhProprietario(proprietario);
      if (resMe.ok) {
        const me = (await resMe.json().catch(() => ({}))) as {
          isMasterAdmin?: boolean;
        };
        setEhMasterAdmin(Boolean(me.isMasterAdmin));
      }
      if (proprietario) {
  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      {!ehProprietario && ehMasterAdmin ? (
        <section className="rounded-lg border border-amber-200 bg-amber-50/80 p-5 dark:border-amber-800 dark:bg-amber-950/35">
          <div className="flex flex-wrap items-center gap-3">
            <Button
              type="button"
              variant="outline"
      ...
      {ehMasterAdmin && modalGdriveAberto ? (
