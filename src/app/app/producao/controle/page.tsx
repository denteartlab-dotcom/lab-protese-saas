...
export default function ControlePage() {
  ...
  useEffect(() => {
    ...
    return () => html.classList.remove("site-zoom-full");
  }, [editando]);
  ...
  }, [editando, form?.repeticao, servicoOsAtualEdicao, colaboradoresOpcoes]);
  ...
  function formVazioEdicao(trabalho: Trabalho, todos: Trabalho[] = trabalhos): EditForm {
    ...
    return {
      ...
      escala: normalizarEscalaOs(categoriasCompletasTabela, trabalho.escala),
      cor: trabalho.cor || "",
  ...
  function selecionarItemEdicao(item: EditItem) {
    ...
    setForm((atual) => ({
      ...
      escala: normalizarEscalaOs(categoriasCompletasTabela, item.categoria || editando!.escala),
      tipoProtese: item.servico,
  ...
  async function salvarEdicao() {
    ...
    if (osFaturada) {
      ...
      if (!res.ok) {
        setErroModalEdicao("Não foi possível gravar observação e comissões.");
        return;
      }
      fecharEdicaoOs();
      void load();
    ...
    if (itensSalvar.length === 0) {
      setErroModalEdicao("Adicione ao menos um serviço, produto ou transporte antes de gravar.");
      return;
    }
    ...
    if (falha) {
      setErroModalEdicao("Não foi possível salvar a OS. Verifique os dados e tente novamente.");
      return;
    }
    ...
    <div className={embedAgenda ? "" : "space-y-3 text-[11px] text-slate-700"}>
      ...
      {editando && form && (
        <div
          className={
            embedAgenda
              ? "modal-fixed-os fixed inset-0 z-50 flex flex-col bg-white"
              : "modal-fixed-os fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 md:p-4"
          }
        >
          <div
            className={
          ...
          >
            ...
            {erroModalEdicao ? (
              <div
                role="alert"
                className="shrink-0 border-b border-red-200 bg-red-50 px-4 py-2 text-xs leading-relaxed text-red-800"
              >
                {erroModalEdicao}
              </div>
            ) : null}
              ...
              <div className="mt-5">
                <div className="mb-3 text-left text-[11px] font-semibold text-emerald-600">
                  ...
                  Etapas
                        ...
                        <button
                          type="button"
                          onClick={() => setAbaServicoEdicao("produtos")}
