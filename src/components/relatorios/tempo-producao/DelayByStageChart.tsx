...
export function DelayByStageChart({ dados }: Props) {
  const chartData = dados.length ? dados : [{ etapa: "Sem dados", dias: 0, os: 0 }];

  return (
    <div className="rounded-xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/90">
      <h3 className="mb-1 text-sm font-semibold text-slate-800 dark:text-slate-100">Atraso médio por etapa</h3>
      <p className="mb-4 text-xs text-slate-500 dark:text-slate-400">Dias de atraso nas OS com prazo vencido</p>
          ...
          <BarChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 48 }}>
            ...
            <Tooltip
              ...
              formatter={(value, _name, item) => [
                `${Number(value ?? 0)} dia(s) — ${item?.payload?.os ?? 0} OS`,
                "Atraso médio",
              ]}
            />
