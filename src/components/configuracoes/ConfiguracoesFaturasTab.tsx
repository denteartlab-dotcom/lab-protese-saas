...
import {
  ...
  normalizarConfiguracoesFaturas,
...
export function ConfiguracoesFaturasTab() {
  ...
  function aplicar(novaConfig: ConfiguracoesFaturas) {
    const normalizado = normalizarConfiguracoesFaturas(novaConfig);
    setConfig(normalizado);
    salvarConfiguracoesFaturas(novaConfig);
    void persistirConfiguracoesFaturasServidor(novaConfig).catch(() => undefined);
  }
  ...
  function alternarPadrao(id: ModeloFaturaId) {
    const ehPadrao = config.modeloPadrao === id;
    const novoPadrao: ModeloFaturaId = ehPadrao
      ? (MODELOS_FATURA_IDS.find((m) => m !== id) ?? "modelo1")
      : id;
    aplicar({ ...config, modeloPadrao: novoPadrao });
  }

  function alternarDuasVias(id: ModeloFaturaId) {
    aplicar({
      ...config,
      duasVias: {
        ...config.duasVias,
