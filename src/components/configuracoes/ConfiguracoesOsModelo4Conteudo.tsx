...
import {
  CAMPOS_MODELO4_GERAL,
  CAMPOS_MODELO4_PARES,
...
function GridCheckboxes({
  ...
  onPatch: (patch: Partial<OsModelo4Layout>) => void;
}) {
  return (
    <div className="space-y-1 border-t border-slate-300/80 pt-2">
      {CAMPOS_MODELO4_PARES.map(([esq, dir], indice) => (
...
export function ConfiguracoesOsModelo4Conteudo() {
  ...
  const [layout, setLayout] = useState<OsModelo4Layout>(() =>
    normalizarOsModelo4Layout(carregarConfiguracoesOs().layoutModelo4)
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setConfigOs(cfgOs);
      setLayout(normalizarOsModelo4Layout(cfgOs.layoutModelo4));
      setCarregando(false);
    })();
  ...
  function patchLayout(patch: Partial<OsModelo4Layout>) {
    setLayout((atual) => normalizarOsModelo4Layout({ ...atual, ...patch }));
  }

  async function salvar() {
    setSalvando(true);
    setMensagem("");
    const layoutNorm = normalizarOsModelo4Layout(layout);
    const novaConfig: ConfiguracoesOs = {
      ...configOs,
      layoutModelo4: layoutNorm,
    };
      ...
      <aside className="flex h-full w-full shrink-0 flex-col border-b border-slate-300 bg-[#d9dde3] lg:w-[360px] lg:border-b-0 lg:border-r">
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-1">
            {CAMPOS_MODELO4_GERAL.map(({ key, label }) => (
              <CheckboxCampo
                key={key}
                label={label}
          ...
          <div className="grid grid-cols-2 gap-x-2 border-t border-slate-300/80 pt-2">
            <CheckboxCampo
              ...
              onChange={(v) =>
                setConfigOs((atual) => ({
                  ...atual,
                  modeloPadrao: v ? "modelo4" : atual.modeloPadrao,
                }))
              }
            />
            <CheckboxCampo
              ...
              onChange={(v) =>
                setConfigOs((atual) => ({
                  ...atual,
                  duasVias: { ...atual.duasVias, modelo4: v },
                }))
              }
            />
          </div>
